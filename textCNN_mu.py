...
class textCNNModel(nn.Module):
    ...
    def __init__(self, pretrained_model, kernel, num_classes):
        ...
        self.embedding_static = torch.nn.Embedding.from_pretrained(embedding_dict)
        self.embedding_non = torch.nn.Embedding.from_pretrained(embedding_dict)
        self.embedding_non.weight.requires_grad = True
        ...
        self.dropout = torch.nn.Dropout(0.64)
    ...
    def init_weight(self):
        ...
        self.MHSA.v.bias.data.zero_()

    def forward(self, x):
        static_embed = self.embedding_static(x)
        embed = self.embedding_non(x)
        x = torch.stack([static_embed, embed], 1)
